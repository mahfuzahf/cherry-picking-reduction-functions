...
# Pre-compile CuPy's internal kernels in the main process before
# any workers are spawned. Workers will then find a valid sm_80 cache
# and won't need to recompile.
_warmup = cp.array([3, 1, 2], dtype=cp.uint64)
_ = cp.unique(_warmup)
del _warmup, _
...
filename = "N_16_alpha_0.95_cost_5_t_80_optimised_kis.pkl"
...
#    public domain." â€” Austin Appleby
...
KERNEL_CODE = r"""
// ============================================================
//  SHA-256  â€”  mochimodev/cuda-hashing-algos (public domain)
// ============================================================
...
// ============================================================
//  MurmurHash3_x86_32  â€”  aappleby/smhasher (public domain)
// ============================================================
...
def worker_task(device_id: int, data_np: np.ndarray, seed_batch: np.ndarray):
    ...
    # Nothing to do if this worker got an empty batch
    if len(seed_batch) == 0:
        return None, -1
...
def get_best_seed_distributed(data_np: np.ndarray, all_seeds: np.ndarray):
    ...
    n_gpus = min(gpus, len(all_seeds))  # never more workers than seeds
    seed_batches = np.array_split(all_seeds, n_gpus)
    ...
    with ctx.Pool(n_gpus) as pool:
        ...
        args = [(i, data_np, seed_batches[i]) for i in range(n_gpus)]
        
        # 3. Parallel Execution
        results_batches = pool.starmap(worker_task, args)
    
    # 4. Find the global winner from the 8 local winners
	# Filter out empty-batch results before taking the max
    results = [(s, c) for s, c in results_batches if s is not None]
