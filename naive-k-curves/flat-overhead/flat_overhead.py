    ...
    try:
        with ctx.Pool(
            ...
            for i in range(t):
                ...
                # update column length for this iteration
                curr_col_len = len(current_column)

                # determine k_i
                # we do 100 cherry-picks in the first column - budget is 2m_0 per column as m_0 to hash, and them m_0*k_0*T_r = m_0 to cherry-pick
                if i == 0:
                    k_i = 100
                else:
                    # for subsequent columns, the budget is 2m_0
                    # as we hash m_i times, the budget to chery-pick is 2m_0 - m_i, and each cherry-pick costs m_i*T_r hashes, so we can afford (2m_0 - m_i) / (m_i * T_r) cherry-picks
                    k_i = floor(((2 * m0) - curr_col_len) / (curr_col_len * T_r))
...
if __name__ == '__main__':
    ...
    for i in range(10):
        ...
        # build the table
        startpoints, final_column, rf_indexes, m_values = build(m0, t)
        ...
        np.save(f"{trial_dir}/kis.npy",          np.array(kis, dtype=np.uint32))
